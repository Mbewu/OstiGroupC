...
of the simulations, a generation consists of ceil(1/f) sub-generations, with f being defined by the user at the GUI.

Importantly, at the start of a subgeneration a death\textunderscore matrix is initialised.  This stores the compostion of the state\textunderscore matrix, i.e. cell types in cellular automaton, at the start of each subgeneration.  

Within each sub generation, a random subset 1/f of non-vessel
automaton elements are selected for updating.  This is acheived by iterating through 1/f of a randomised $N$*$M$ vector consisting of a number corresponding to each cell.  On entrance of the subgeneration function it is first checked whether the iteration of the current subgeneration will access elements with the $N$ by $M$ matrix.  This issue occurs as f, which defines the number of subgeneration and is controlled by the user, can be set so as to create subgeneration cell amounts that do not factor into in $N$ by $M$.  Therefore if the last subgeneration iteration extends beyond the $N$ by $M$ it must be aborted.  This means cell within that final subgeneration will not be updated, but is a ocnsequence of the ability to modulate f in the current scheme.  An improvement to the code would be to iterate the value of remaining subgeneration, that was not possible in this project due to time constraints.  This randomised $N$ by $M$ vector is reinitialised at the beginning of every generation. The update follows the following order: 
\begin{enumerate}
\item The entire glucose and lactic acid fields are updated according to the solution of the equilibrium boundary value
problems. 
\item Each cell from the subgeneration is applied to the cellular automaton. The cell is partioned into a normal or cancerous cell depending on its type.
\item The cell is then converted/maintained as quiescient, dead or active depending on the surrounding (cell right, left, up and down) pH and glucose fields. 
\item Following this, the death\textunderscore matrix, a representation of the state\textunderscore matrix at the start of the subgeneration is used to decide whether active cells from the current subgeneration state\textunderscore matrix can divide into free space created by dead cells from the previous subgeneration. This scheme means cells that have died in the current subgeneration cannot be occupied in the same subgeneration.  At the end of the subgeneration the death matrix is updated to the current state\textunderscore matrix, i.e. with the subgeneration changes, meaning that active cells in the next subgeneration are free to divide into the free space.  
\end{enumerate}
